// Written by Liam Kinney, kinne351 (just the pawnPromotion method)
public class Piece {
    ...
    public static int[] pawnPromotion(Board board){ // Static so it can be accessed in game class
        for(int i = 0; i < 8; i++) { // 8 because the rows are of length 8
            // If a white pawn is anywhere on row 0 (first row) or a black pawn on row 7 (last row), then there
            // will be pawn promotion and the location of that pawn is returned as an int[]
            if(board.getPiece(0, i) != null && board.getPiece(0, i).getCharacter() == '\u2659')
                return new int[]{0, i};
            if(board.getPiece(7, i) != null && board.getPiece(7, i).getCharacter() == '\u265f')
                return new int[]{7, i};
        }
        return new int[]{-1, -1};
    }
